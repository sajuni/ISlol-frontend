    ...
    <template #content>
      <section class="detail_area">
        <section class="title_info">
          ...
          <section class="detail_date">
            <span>작성자: {{ notice.member.memberNick }}</span>
        ...
        <b-button class="detail_button" variant="primary" @click="goList()"
          >목록</b-button
        >
        <b-button class="mx-3" v-if="role" @click="goUpdate(notice.noticeSeq)"
          >수정</b-button
        >
      </section>
    </template>
...
export default {
  ...
  methods: {
    goList() {
      ...
      this.$router.push({
        name: 'NoticeList',
        params: { page: this.$store.getters['notice/getCurrentPage'] },
      });
    },
    goUpdate(noticeSeq) {
