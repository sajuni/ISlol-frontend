<template>
  <body>
    ...
    <section class="main ls_board">
      <section class="main ls_photo">
        <h4>포토</h4>
        <section class="ls_img">
          <b-img
        ...
        <section>
          ...
          >
            <template #cell(content)="data">
              <router-link
                :to="{ path: `/notice/detail/${data.item.noticeSeq}` }"
  ...
  methods: {
    ...
    getMediaList() {
      this.$store.dispatch('media/getList').then(() => {
        ...
        this.videoList = this.makeRankList(
          this.$store.getters['media/getVideoList']
        );
        this.imageList = this.makeRankList(
          this.$store.getters['media/getImageList']
        );
      });
    },
    makeRankList(arg) {
